...
async function applyWizardTemplate(templateId, devices, customPresets = {}) {
  ...
  for (const wizardConfig of template.wizards) {
    // Find applicable devices for this wizard
    const applicableDevices = devices.filter(device => {
      // You may want to adjust this logic based on your device/wizard matching
      return true; // Placeholder: all devices are applicable
    });

    // Build discovery context/defaults if needed (placeholder logic)
    const discoveryContext = null;
    ...
    if (wizardConfig.autoExecute) {
      try {
        ...
        const presets = {};
        if (template.presets && template.presets[wizardConfig.id]) {
          Object.assign(presets, template.presets[wizardConfig.id]);
        }
        if (customPresets && customPresets[wizardConfig.id]) {
          Object.assign(presets, customPresets[wizardConfig.id]);
        }
